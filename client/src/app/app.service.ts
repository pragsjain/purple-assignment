...
export class AppService {
  ...
  getAllUsers(): Observable<any> {
   ...
   getAllProducts(): Observable<any> {
    return this.http.get(`${this.url}/api/v1/products/all`)
    .pipe(map((response:any) =>{
        return response;
    }),catchError(<T>(error: any, result?: T) => {
      console.log(error);
   ...
   getProductById(id): Observable<any> {
    return this.http.get(`${this.url}/api/v1/products/view/${id}`);
  } 
    
  createProduct(formdata): Observable<any> {
    return this.http.post(`${this.url}/api/v1/products/create`, formdata);
  } 
    
  editProduct(data): Observable<any> {
    return this.http.put(`${this.url}/api/v1/products/${data['id']}/edit`, data);
  } 

  deleteProduct(id): Observable<any> {
    return this.http.post(`${this.url}/api/v1/products/${id}/delete`,id);
  } 
